...
import { missingBranches } from './lib/gen';
...
declare global {
  namespace jest {
    ...
    interface Matchers<R, T> {
       ...
       * been removed. It is intended to assert that a function is resilient against
       * incomplete or invalid data received from external sources.
       *
       * See /packages/expect-more-jest/test/gen/missing-branches.spec.ts for detail
       * on how values are deconstructed.
       *
       * @example
       ...
       * });
       */
      toHandleMissingBranches(): R;
    }
  }
}

export const toHandleMissingBranchesMatcher = createToHandleComparer('toHandleMissingBranches', missingBranches);

expect.extend({ toHandleMissingBranches: toHandleMissingBranchesMatcher });
